...
class ParticleFilter(Node):
    """Filtro de partículas para localización Monte Carlo (MCL)"""
    def __init__(self):
    ...
    def initialize_particles(self):
        self.get_logger().info("Inicializando partículas en espacio libre...")
        
        self.particles = []
        attempts = 0
        max_attempts = 10000
        
        max_x = self.map_info['width'] * self.map_info['resolution']
        max_y = self.map_info['height'] * self.map_info['resolution']
        margin = 0.1
        
        while len(self.particles) < self.num_particles and attempts < max_attempts:
            x = uniform(margin, max_x - margin)
            y = uniform(margin, max_y - margin)
        ...
        while len(self.particles) < self.num_particles:
            ...
            ang = uniform(-np.pi, np.pi)
            self.particles.append(Particle(x, y, ang, self.sigma_motion))
            self.get_logger().warn("Partícula colocada sin verificar espacio libre")
        
        for particle in self.particles:
            particle.weight = 1.0 / self.num_particles
        
        self.get_logger().info(f"Inicializadas {len(self.particles)} partículas en espacio libre")
        self.get_logger().info(f"Intentos necesarios: {attempts}")
    ...
    def is_position_free(self, x, y):
        ...
        if not (0 <= px < self.map_info['width'] and 0 <= py < self.map_info['height']):
            return False
        
        if self.map_data[py, px] > 200:
            return True
